...
RUNTIME_DIR = BASE_DIR / "main_ncc_oper_runtime"
...
logger = logging.getLogger("MAIN_NCC_OPER")
...
def mark_regions_report_sent(regions):
    ...
    for region in regions:
        if region in state:
            state[region]["last_report_sent_at"] = sent_at
...
def get_font_paths():
    local_regular = BASE_DIR / "fonts" / "DejaVuSans.ttf"
    local_bold = BASE_DIR / "fonts" / "DejaVuSans-Bold.ttf"
...
async def fetch_alarms():
    ...
    try:
        async with aiohttp.ClientSession() as http_session:
            async with http_session.get(API_URL, headers=headers, ssl=False) as resp:
                if resp.status == 200:
...
def generate_image(stats, regions):
    ...
    logger.info("GENERATE IMAGE | regions=%s", regions)

    rows = ["Время МО ЭП", "Нет ЭП", "ДГ", "МГ", "Кол-во недоступных БС"]

    font_regular_path, font_bold_path = get_font_paths()
    try:
        if font_regular_path and font_bold_path:
            font = ImageFont.truetype(str(font_regular_path), 22)
            font_bold = ImageFont.truetype(str(font_bold_path), 22)
        else:
            raise OSError("Font files not found")
    except Exception:
    ...
    def draw_center(draw, x, y, w, h, text, bold=False):
        ...
        text_x = x + (w - width) // 2
        text_y = y + (h - 18) // 2
        used_font = font_bold if bold else font
        draw.text((text_x, text_y), text, fill="black", font=used_font)
